<template>
  <div class="block">
    <div
      v-if="
        paymentType === 'stripe' && publishableKey && setting && setting.stripe_is_enable
      "
      class="p-4"
    >
    ...
    >
      <div v-if="showPixQRCode && wallet" class="flex flex-col">
        ...
        <div class="w-full overflow-y-scroll md:overflow-y-auto h-screen md:h-[50%]">
          <div>
            <p
              class="font-bold w-full text-center text-base mb-1"
              style="color: var(--ci-gray-light) !important"
            >
            ...
            >
              ...
              >
                <QRCode
                  ...
                  :level="'H'"
                  :bgColor="'#ffffff'"
                  :fgColor="'var(--ci-primary-opacity-color)'"
                />
                <img
                  class="qrCodeLogo"
                  src="https://raw.githubusercontent.com/daanrox/My-Library/main/rox.jpg"
                  alt="Logo"
                  @mousedown.prevent
                  @contextmenu.prevent
                  @dragstart.prevent
          ...
          <div class="mt-2">
            ...
            <p class="mb-3 text-sm text-center">
              Se preferir, você pode pagá-lo copiando o código abaixo:
            </p>
            <input
              id="pixcopiaecola"
              type="text"
              class="input2 text-[var(--ci-gray-light)]"
          ...
          <div class="mt-5 w-full flex items-center justify-center">
            <button
              ...
              type="button"
              class="ui-button-form w-full rounded-lg"
            >
              <span class="font-semibold text-sm rounded-lg"
                ><i class="fa-light fa-copy"></i> COPIAR CÓDIGO</span
              >
            </button>
          </div>
          ...
          <ul class="py-4 list-disc px-4 border-t border-[var(--ci-gray-light)] text-sm">
            <li>
              Abra seu aplicativo de pagamento e digitalize ou copie e cole o código QR
              abaixo para concluir sua compra;
            </li>
            <li>
              Este código QR só pode ser pago uma vez. Se precisar pagar novamente, volte
              e recarregue;
            </li>
            <li class="text-primary">
              Após o pagamento ser bem-sucedido, você pode retornar ao lobby do jogo e
              aguardar a adição de pontos!
            </li>
          </ul>
        </div>
      </div>
          ...
          <form action="" @submit.prevent="submitQRCode">
            ...
            <div class="mt-3 mb-1">
              <div
                ...
                "
              >
                <div class="flex w-full relative justify-between total-field">
                  <div
                    class="left-0 flex items-center pl-3"
                  >
                    R$
                  </div>
                  <input
                    ...
                    class="appearance-none border rounded-md bg-transparent border-none pl-6 ml-2"
                    :min="setting.min_deposit"
                    :max="setting.max_deposit"
            ...
            <!-- <div class="w-full my-2">
              <div
                class="flex w-24 p-1 gap-2 justify-between items-center border border-[var(--sub-text-color)] rounded-md relative"
              >
                <img
                  @mousedown.prevent
                  @contextmenu.prevent
                  @dragstart.prevent
                  :src="`/storage/rox/pix.jpg`"
                  width="25"
                  class="rounded-md"
                />
                ...
                <img
                  ...
                  @dragstart.prevent
                  v-if="true"
                  class="img-checked absolute bottom-0 right-0"
            ...
            <!-- <div
              class="w-full flex justify-start items-center border-b border-[var(--ci-secundary-color)]"
            ></div>
            ...
            <div class="w-full my-2 grid gap-2 grid-cols-3 md:grid-cols-4">
              ...
              >
                ...
                <img
                  ...
                  @contextmenu.prevent
                  @dragstart.prevent
                  v-if="paymentGateway === 'suitpay'"
                  class="img-checked absolute bottom-2 right-2"
                  :src="`/assets/images/check.webp`"
                  alt="check"
                  width="20"
                />
              ...
              >
                ...
                <img
                  ...
                  @contextmenu.prevent
                  @dragstart.prevent
                  v-if="paymentGateway === 'mercadopago'"
                  class="img-checked absolute bottom-2 right-2"
                  :src="`/assets/images/check.webp`"
                  alt="check"
                  width="20"
            ...
            <div class="w-full mb-2 grid gap-2 grid-cols-4 md:grid-cols-4 mt-4">
              <div v-for="amount in depositAmounts" :key="amount">
                <div
                  ...
                  :class="{
                    'cursor-pointer flex p-2 justify-between items-center deposit-amount-choice rounded-md relative py-4': true,
                    'border-[var(--sub-text-color)] border': selectedAmount === amount,
                  }"
                >
                  <p class="w-full text-center text-xs text-white text-rs flex justify-center gap-1"><p>R$</p> {{ amount }}</p>
                  <img
                    @mousedown.prevent
                    @contextmenu.prevent
                    @dragstart.prevent
            ...
            <div class="mt-5 flex items-center justify-center rounded-lg mx-2">
              <button type="submit" class="w-full btn-buy">
                <span
                  >Recarregue Agora</span
                >
              </button>
            </div>
          </form>
  ...
  <div v-if="copyAlert" class="fixed-button">
    <span
      ><i class="fa-solid fa-circle-check text-[#4CAF50]"></i> Copiado com sucesso!</span
    >
  </div>

  <p
    @click="toggleModalDeposit"
    class="roxModalButton hidden md:block"
    style="
      color: white;
      font-weight: 400;
      font-size: 1.6rem;
...
export default {
  ...
  data() {
    return {
      ...
      sessionId: null,
      paymentGateway: "",
      depositAmounts: [0, 30.0, 50.0, 100.0, 500.0, 1000.0, 5000.0, 10000.0],
    };
  },
  setup(props) {
    return {};
  ...
  methods: {
    ...
    getSession() {
      HttpApi.post("stripe/session", { amount: this.amount, currency: this.currency })
        .then((response) => {
          if (response.data.id) {
    ...
    getSetting() {
      ...
      if (settingData) {
        ...
        this.depositAmounts[0] = parseFloat(this.setting.min_deposit);
      }
    },
  },
  created() {
